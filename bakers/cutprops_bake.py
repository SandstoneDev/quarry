...
# native bit 0x01000000) - the codec the ambient peds need (task #36) - so each is skipped
# and a valid (possibly empty) CPRP container is written. Defaults keep the PC dev loop.
...
def bake_mesh(img, dffname, txdname):
    ...
    if _is_native_dff(raw):
        raise SystemExit("PS2-native VIF geometry (codec #36 pending)")
    if parse_dff is None:
        raise SystemExit("formats.dff parser unavailable in this tree")
    dff = parse_dff(raw)
