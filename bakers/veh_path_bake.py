#!/usr/bin/env python3
"""veh_path_bake - SA vehicle path nodes (NODES*.DAT) -> per-region vehpath tiles.

Near-clone of ped_path_bake, but keeps the VEHICLE nodes (the first
num_vehicle_nodes of each file) instead of the ped nodes. These are the road
graph traffic cruises along (CarCtrl / CAutoPilot FollowPath). Sliced into the
SAME region grid as the target chunk set (origin/tile from its regions.bin), so
...
def main():
    ...
    # ---- load all vehicle nodes globally ----
    ...
    for path in sorted(glob.glob(os.path.join(ROOT_PC, "data/Paths/NODES*.DAT")) +
        ...
        nd = parse_nodes(data)
        for n in nd.nodes[:nd.num_vehicle_nodes]:          # VEHICLE nodes only
            links = []
            ...
            order.append(key)
    print("vehicle nodes: %d" % len(by_addr))
    ...
    for (rx, ry), keys in sorted(tiles.items()):
        ...
        for key in keys:
            ...
            nodes_blob += struct.pack("<3f4BHH", x, y, z, w8, nd["prob"], flags, cnt,
                                      first, 0)
        buf = b"VPTH" + struct.pack("<HH", len(keys), nlinks_total) + nodes_blob + links_blob
        open(os.path.join(out, "vehpath_%d_%d.bin" % (rx, ry)), "wb").write(buf)
